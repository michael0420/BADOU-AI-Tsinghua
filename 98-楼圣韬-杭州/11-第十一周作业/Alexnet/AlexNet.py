...
class AlexNet(torch.nn.Module):
    def __init__(self,num_classes=NUM_CLASSES) -> None:
        ...
        # 根据AlexNet论文结构创建Sequential容器
        self.features = nn.Sequential(
            nn.Conv2d(3,64,kernel_size=3,stride=2,padding=1),  # (3,32,32)->(64,16,16)
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=2),      # (64,16,16)->(64,8,8)
            nn.Conv2d(64,192,kernel_size=3,padding=1),  # (64,8,8)->(192,8,8)
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=2),  # (192,8,8)->(192,4,4)
            nn.Conv2d(192,384,kernel_size=3,padding=1), # (192,4,4)->(384,4,4)
            nn.ReLU(inplace=True),
            nn.Conv2d(384,256,kernel_size=3,padding=1),# (384,4,4)->(256,4,4)
            nn.ReLU(inplace=True),
            nn.Conv2d(256,256,kernel_size=3,padding=1),# (256,4,4)->(256,4,4)
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=2) # (256,4,4)->(256,2,2)
        )
        # 设置两个全连接层
        self.classifier = nn.Sequential(
            nn.Dropout(), # 为了防止过拟合，用Dropout舍弃一部分元素
            nn.Linear(256*2*2,4096),  # (256,2,2) 展开变为全连接层1024个神经元
            nn.Dropout(),
            nn.Linear(4096,4096),  # 第二个全连接层
            nn.ReLU(inplace=True),
            nn.Linear(4096,num_classes), # 最后输出 Num_classes 
        )
    ...
    def forward(self,x):
        ...
        x = self.features(x)
        x = x.view(x.size(0),256*2*2) # reshape成256*2*2
        x=self.classifier(x)
        return x
