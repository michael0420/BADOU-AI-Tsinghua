...
def myMobileNet(input_shape=[224,224,3],
                depth_multiplier=1,
                dropout=1e-3,
                classes=1000
                ):
    ...
    x = depthwise_conv_block(x,256,depth_multiplier,strides=(2,2),block_id=6)
    ...
    inputs = input_shape
    model = myMobileNet(inputs,x,name='myMobileNet')
    weights_name = 'mobilenet_1_0_224_tf.h5'
...
def depthwise_conv_block(inputs,pointswise_conv_filters,depth_multiplier=1,strides=(1,1),block_id=1):
    ...
    x = Activation(relu6,name='conv_dw_%d_relu')(x)

    x = Conv2D(pointswise_conv_filters,(1,1),padding='same',use_bias=False,strides=(1,1),name='conv_pw_%d'%block_id)(x)
...
if __name__ == '__main__':
    model = myMobileNet(input_shape=(224, 224, 3))
