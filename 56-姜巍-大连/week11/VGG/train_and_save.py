...
from MyallconvVGG16_net import MyVgg16  # 导入自定义神经网络
import torch
...
if __name__ == '__main__':
    ...
    net = MyVgg16().to(device)  # 实例化神经网络，存入GPU
    ...
    for epoch in range(epoches):  # 数据被遍历的次数
        ...
        for i, data in enumerate(trainloader, 0):
            ...
            if i % 90 == 89:  # 每90个mini batches输出一次
                print(f'[epoch:{epoch + 1}, mini-batch:{i + 1:3d}] loss: {running_loss / 90:.5f}')
                running_loss = 0.0

    print('Finished Training !')

    PATH = './trained_VGG_net.pth'
