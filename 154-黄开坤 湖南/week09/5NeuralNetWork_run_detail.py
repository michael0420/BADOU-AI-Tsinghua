...
print(targets)  #target第8个元素的值是0.99，这表示图片对应的数字是7(数组是从编号0开始的).
...
training_data_list = training_data_file.readlines() #一张图片的数据是新建数组的一个元素
training_data_file.close()
# 把数据依靠‘,’，区分，并分别读入
for record in training_data_list:
    all_values = record.split(',')  # 切分
...
for record in test_data_list:
    ...
    print('gai图片对应的数字为：', correct_number)
    #预处理数字图片
    inputs = (np.asfarray(all_values[1:])) / 255 * 0.99 + 0.01
    #让网络判断图片对应的数字，推理
...
print('perfermance = ', scores_array.sum() / scores_array.size) #size()函数主要是用来统计矩阵元素个数，或矩阵某一维上的元素个数的函数。
# [4]
'''
在原来网络训练的基础上再加上一层外循环
但是对于普通电脑而言执行的时间会很长。
