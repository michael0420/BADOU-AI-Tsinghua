...
'''
2.利用高斯分布公式 μ=0 σ=0.5  1/(sqrt(2π)*σ) * exp((-x**2)/2*σ**2) 计算出kernel
通过kernel完成高斯模糊,达到降噪的目的
'''
...
img_tidu_x[img_tidu_x == 0] = 0.00000001  # 分母不能为0
angle = img_tidu_y / img_tidu_x  # angle这个变量留着线性插值用,个人感觉这个变量名叫scale比较好
plt.figure(2)
...
for i in range(1, dx - 1):
    for j in range(1, dy - 1):
        ...
        if angle[i, j] <= -1:  # 90度-135度 看 1 2 和 8 9
            num_1 = (temp[0, 1] - temp[0, 0]) / angle[i, j] + temp[0, 1]
            num_2 = (temp[2, 1] - temp[2, 2]) / angle[i, j] + temp[2, 1]
            if not (img_tidu[i, j] > num_1 and img_tidu[i, j] > num_2):
                flag = False
        elif angle[i, j] >= 1:  # 45度-90度 看 2 3 和 7 8
            num_1 = (temp[0, 2] - temp[0, 1]) / angle[i, j] + temp[0, 1]
            num_2 = (temp[2, 0] - temp[2, 1]) / angle[i, j] + temp[2, 1]
            if not (img_tidu[i, j] > num_1 and img_tidu[i, j] > num_2):
                flag = False
        elif angle[i, j] > 0:  # 0度-45度 看 3 6 和 4 7
            num_1 = (temp[0, 2] - temp[1, 2]) * angle[i, j] + temp[1, 2]
            num_2 = (temp[2, 0] - temp[1, 0]) * angle[i, j] + temp[1, 0]
            if not (img_tidu[i, j] > num_1 and img_tidu[i, j] > num_2):
                flag = False
        elif angle[i, j] < 0:  # 135度-180度 看 1 4 和 6 9
...
'''
4、双阈值检测，threshold_1 threshold_2
遍历所有阈值大于threshold_2的点,查看8邻域是否存在range[threshold_1,threshold_2]之间的点
'''
...
# 每一个确定是边缘的像素,8邻域都遍历一遍,看看还有没有其他边缘像素
